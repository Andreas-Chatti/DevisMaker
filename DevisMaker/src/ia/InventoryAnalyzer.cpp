...
{
    if (m_ia->getFallbackAttempts() >= m_ia->getMaxFallbackAttempts())
    {
        emit analysisError("Les trois tentatives de l'IA ont echoues");
        m_ia->clearFallbackAttempts();
        return;
    }

    m_ia->addFallbackAttempt();
